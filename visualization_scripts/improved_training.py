...
class ImprovedTerrainTrainer:
    ...
    def create_environment(self) -> TerrainGridNavEnv:
        ...
        env = TerrainGridNavEnv(
            ...
            slope_penalty_weight=0.01,  # 大幅降低坡度惩罚
            height_penalty_weight=0.005,  # 大幅降低高度惩罚
    ...
    def train(self, num_episodes: int = 1000, update_interval: int = 20, 
        ...
        for episode in range(num_episodes):
            ...
            if (episode + 1) % save_interval == 0:
                ...
                self.save_model(f"improved_terrain_ppo_model_ep{episode+1}.pth")
        ...
        self.save_model("improved_terrain_ppo_model_final.pth")
    ...
    def save_training_data(self, episode: int, is_final: bool = False):
        ...
        stats_file = os.path.join(self.data_dir, f"improved_training_stats_{suffix}.json")
    ...
    def test_final_performance(self, num_tests: int = 50):
        ...
        test_file = os.path.join(self.data_dir, "improved_final_test_results.json")
...
def main():
    ...
    print("改进版真实地形PPO训练 - 调整奖励函数")
    print(f"地形文件: {terrain_file}")
    print(f"起点: {start_point}")
    print(f"终点: {goal_point}")
    
    # 创建训练器
    trainer = ImprovedTerrainTrainer(terrain_file, start_point, goal_point)
    
    # 开始训练（先用较少的episodes测试）
    trainer.train(
