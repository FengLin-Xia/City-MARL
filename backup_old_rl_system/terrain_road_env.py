...
class TerrainRoadEnvironment(gym.Env):
    ...
    def _setup_spaces(self):
        ...
        self.observation_space = spaces.Dict({
            'height_map': spaces.Box(low=0, high=100, shape=self.grid_size, dtype=np.float32),
            'terrain_map': spaces.Box(low=0, high=5, shape=self.grid_size, dtype=np.int32),
            'road_map': spaces.Box(low=0, high=3, shape=self.grid_size, dtype=np.int32),
            'agent_pos': spaces.Box(low=0, high=max(self.grid_size), shape=(2,), dtype=np.int32),
            'target_pos': spaces.Box(low=0, high=max(self.grid_size), shape=(2,), dtype=np.int32),
        ...
        try:
            if isinstance(mesh_data, np.ndarray):
                ...
                if len(mesh_data.shape) == 3:
                    ...
                    # 先设置grid_size
                    if self.grid_size is None:
                        self.grid_size = self.height_map.shape
                        print(f"📏 设置网格尺寸为: {self.grid_size}")
                ...
                # 如果有水体信息，使用它来生成地形图
                if 'water_mask' in mesh_data:
                    water_mask = np.array(mesh_data['water_mask'])
                    if water_mask.size > 0:
                        self._generate_terrain_from_height_and_water(water_mask)
                    else:
                        self._generate_terrain_from_height()
                else:
                    self._generate_terrain_from_height()
            ...
            print(f"❌ 处理mesh数据时出错: {e}")
            print(f"   高程图形状: {self.height_map.shape if hasattr(self, 'height_map') else 'None'}")
            if 'water_mask' in mesh_data:
                water_mask = np.array(mesh_data['water_mask'])
                print(f"   水体掩码形状: {water_mask.shape}")
                print(f"   水体掩码类型: {water_mask.dtype}")
                print(f"   水体掩码内容示例: {water_mask[:5, :5]}")
            raise
    ...
    def _generate_terrain_from_height_and_water(self, water_mask):
        """根据高程和水体信息生成地形类型"""
        self.terrain_map = np.zeros(self.grid_size, dtype=np.int32)
        
        # 确保water_mask是正确的形状
        if water_mask.shape != self.grid_size:
            print(f"⚠️ 水体掩码形状不匹配: {water_mask.shape} vs {self.grid_size}")
            # 如果形状不匹配，回退到基于高程的分类
            self._generate_terrain_from_height()
            return
        
        # 首先标记水体
        self.terrain_map[water_mask] = TerrainType.WATER.value
        ...
        if len(land_heights) > 0:
            height_percentiles = np.percentile(land_heights, [25, 50, 75])
            
            # 对陆地分类
            land_indices = np.where(land_mask)
            for i in range(len(land_indices[0])):
                y, x = land_indices[0][i], land_indices[1][i]
                height = self.height_map[y, x]
                
                if height < height_percentiles[0]:
                    self.terrain_map[y, x] = TerrainType.GRASS.value
                elif height < height_percentiles[1]:
                    self.terrain_map[y, x] = TerrainType.FOREST.value
                elif height < height_percentiles[2]:
                    self.terrain_map[y, x] = TerrainType.MOUNTAIN.value
                else:
                    self.terrain_map[y, x] = TerrainType.BUILDING.value
