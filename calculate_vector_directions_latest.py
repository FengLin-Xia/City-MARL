...
# -*- coding: utf-8 -*-
"""
计算向量方向脚本 - 使用repulsion_outputs中最新的12个地块点数据
...
class VectorDirectionCalculator:
    ...
    def find_latest_csv_files(self, repulsion_dir: str):
        """
        找到repulsion_outputs中最新的12个CSV文件（每个地块一个）
        ...
        Returns:
            最新的12个CSV文件路径列表
        """
        print(f"正在查找最新的12个CSV文件: {repulsion_dir}")
        
        # 查找所有parcel_*_final_points_*.csv文件
        ...
        for csv_file in csv_files:
            ...
            # 解析文件名: parcel_{id}_final_points_{timestamp}.csv
            match = re.match(r'parcel_(\d+)_final_points_(\d{8}_\d{6})\.csv', filename)
            if match:
                parcel_id = int(match.group(1))
                timestamp = match.group(2)
                file_info.append((parcel_id, timestamp, csv_file))
        ...
        # 为每个地块ID找到最新的文件
        latest_files = {}
        for parcel_id, timestamp, csv_file in file_info:
            if parcel_id not in latest_files:
                latest_files[parcel_id] = csv_file
        
        # 只取前12个地块（0-11）
        result_files = []
        for parcel_id in range(12):
            if parcel_id in latest_files:
                result_files.append(latest_files[parcel_id])
                print(f"地块 {parcel_id}: {os.path.basename(latest_files[parcel_id])}")
        
        print(f"成功找到 {len(result_files)} 个最新地块文件")
        return result_files
    
    def load_latest_repulsion_points(self, repulsion_dir: str):
        """
        加载最新的12个地块点数据
        ...
        """
        print(f"正在加载最新地块数据: {repulsion_dir}")
        
        # 找到最新的CSV文件
        latest_files = self.find_latest_csv_files(repulsion_dir)
        ...
        for csv_file in latest_files:
            # 从文件名提取地块ID
            filename = os.path.basename(csv_file)
            match = re.match(r'parcel_(\d+)_final_points_(\d+)\.csv', filename)
            if match:
                parcel_id = int(match.group(1))
            else:
                parcel_id = 0
...
def main():
    ...
    try:
        points = calculator.load_latest_repulsion_points(repulsion_dir)
    ...
    # 保存结果
    output_file = "vector_directions_latest.txt"
