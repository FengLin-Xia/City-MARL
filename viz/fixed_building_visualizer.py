#!/usr/bin/env python3
"""
Fixed Building Visualizer for v2.3
修复版本的可视化器，解决matplotlib警告和等值线绘制问题
"""
...
class FixedBuildingVisualizer:
    ...
    def setup_ui(self):
        """设置用户界面"""
        # 主标题
        self.fig.suptitle('Enhanced City Simulation v2.3 - Fixed Building Visualization', 
    ...
    def show_frame(self, frame_index: int):
        ...
        # 设置标题
        month = frame_index
        self.ax.set_title(f'Month {month:02d} - Fixed Building Visualization', fontsize=14)
    ...
    def draw_buildings(self, frame_index: int):
        """绘制建筑"""
        if frame_index not in self.building_data:
            return
        
        data = self.building_data[frame_index]
    ...
    def draw_isocontours(self, frame_index: int):
        ...
        for level in commercial_levels:
            try:
                ...
                if np.min(sdf_field) <= level <= np.max(sdf_field):
                    contour = self.ax.contour(X, Y, sdf_field, levels=[level], colors='red', 
        ...
        for level in residential_levels:
            try:
                # 检查等值线是否存在
                if np.min(sdf_field) <= level <= np.max(sdf_field):
                    contour = self.ax.contour(X, Y, sdf_field, levels=[level], colors='blue', 
                                            linestyles='dashed', alpha=0.6, linewidths=1)
            except Exception as e:
    ...
    def show_building_stats(self, month: int):
        """显示建筑统计"""
        if month in self.building_data:
...
def main():
    """主函数"""
    print("🏙️ Fixed Building Visualizer v2.3")
    print("=" * 50)
    print("🎯 Features:")
    print("  • Fixed matplotlib warnings")
    print("  • Improved isocontour drawing")
    print("  • Direct building visualization")
    print("  • Interactive playback controls")
    print("=" * 50)
    
    # 创建并显示可视化器
    visualizer = FixedBuildingVisualizer()
    plt.show()
