...
def visualize_vector_field(points, angles, parcel_ids, hub2_x=112, hub2_y=121, output_file="latest_vector_directions_visualization.png"):
    """可视化向量场"""
    ...
    for i, parcel_id in enumerate(unique_parcels):
        ...
        ax1.scatter(x_coords[mask], y_coords[mask], c=[colors[i]], s=30, alpha=0.8, label=f'Parcel {parcel_id}')
    ...
    # 右上图：散点图显示角度分布（按角度着色）
    scatter = ax2.scatter(x_coords, y_coords, c=angles_array, cmap='hsv', s=30, alpha=0.8)
    ...
    # 左下图：向量场箭头图（所有点）
    # 为了清晰显示，只显示部分向量
    step = max(1, len(points) // 100)  # 最多显示100个箭头
    ...
    ax3.quiver(x_arrows, y_arrows, u, v, angles=angle_arrows, scale=30, alpha=0.8)
    ...
    # 右下图：角度分布直方图
    ax4.hist(angles_array, bins=30, alpha=0.7, edgecolor='black')
    ax4.set_xlabel('角度 (度)')
    ax4.set_ylabel('频次')
...
def main():
    """主函数"""
    print("=== 最新Repulsion向量方向可视化 ===")
    
    # 加载数据
    points, angles, parcel_ids = load_vector_directions("vector_directions_latest.txt")
