...
"""
Alpha Shape边界地形处理器
使用Alpha Shape提取真实的mesh边界，保留凹陷和复杂形状
"""
...
class AlphaShapeProcessor:
    ...
    def extract_alpha_boundary(self, alpha: float = 0.5) -> List[Tuple[float, float]]:
        """使用Alpha Shape提取真实边界"""
        if self.vertices is None:
            return None
        
        print(f"🔄 使用Alpha Shape提取边界 (alpha={alpha})...")
        
        try:
            # 尝试导入alphashape
            from alphashape import alphashape
        except ImportError:
            print("❌ 需要安装alphashape库: pip install alphashape")
            print("   使用凸包作为备选方案...")
            return self._extract_convex_hull_boundary()
        ...
        try:
            # 使用Alpha Shape提取边界
            alpha_shape = alphashape(points_2d, alpha=alpha)
            
            # 提取边界坐标
            if hasattr(alpha_shape, 'exterior'):
                # 多边形
                boundary_coords = list(alpha_shape.exterior.coords)
            elif hasattr(alpha_shape, 'coords'):
                # 线串
                boundary_coords = list(alpha_shape.coords)
            else:
                print("❌ Alpha Shape提取失败，使用凸包")
                return self._extract_convex_hull_boundary()
            ...
            print(f"   Alpha Shape边界点数量: {len(boundary_coords)}")
            self.boundary_points = boundary_coords
            
            return boundary_coords
            
        except Exception as e:
            print(f"❌ Alpha Shape提取失败: {e}")
            print("   使用凸包作为备选方案...")
            return self._extract_convex_hull_boundary()
    
    def _extract_convex_hull_boundary(self) -> List[Tuple[float, float]]:
        """提取凸包边界（备选方案）"""
        ...
        for idx in boundary_indices:
            boundary_points.append((x_coords[idx], y_coords[idx]))
        
        print(f"   凸包边界点数量: {len(boundary_points)}")
        self.boundary_points = boundary_points
        
        return boundary_points
    
    def create_alpha_mask(self, grid_size: Tuple[int, int]) -> np.ndarray:
        """使用Alpha Shape边界创建掩码"""
    ...
    def process_terrain(self, obj_filepath: str, grid_size: Tuple[int, int] = (150, 150), alpha: float = 0.5) -> Dict:
        """处理地形数据"""
        print("🚀 开始Alpha Shape边界地形处理")
        print("=" * 50)
        ...
        # 2. 提取Alpha Shape边界
        boundary_points = self.extract_alpha_boundary(alpha)
        if boundary_points is None:
            return None
        
        # 3. 创建Alpha Shape掩码
        mask = self.create_alpha_mask(grid_size)
        if mask is None:
            return None
    ...
    def visualize_result(self, save_path: Optional[str] = None):
        ...
        fig.suptitle('Alpha Shape边界地形处理结果', fontsize=16)
        ...
        if self.vertices is not None:
            ...
            if self.boundary_points:
                ...
                axes[0, 0].plot(boundary_x, boundary_y, 'r-', linewidth=2, label='Alpha Shape边界')
        ...
        axes[0, 1].set_title('Alpha Shape边界掩码')
        ...
        axes[1, 0].set_title('高程图（Alpha Shape边界）')
...
def main():
    ...
    # 创建处理器
    processor = AlphaShapeProcessor()
    
    # 处理地形（可以调整alpha值）
    result = processor.process_terrain(obj_filepath, grid_size=(150, 150), alpha=0.5)
    ...
    output_file = os.path.join(output_dir, "terrain_alpha_shape.json")
    ...
    processor.visualize_result(
        save_path="visualization_output/alpha_shape_terrain.png"
    )
